ALTER TABLE IF EXISTS public.ufc_full_fight_card
  ADD COLUMN IF NOT EXISTS "KO_TKO_Wins" integer,
  ADD COLUMN IF NOT EXISTS "KO_TKO_Losses" integer,
  ADD COLUMN IF NOT EXISTS "Submission_Wins" integer,
  ADD COLUMN IF NOT EXISTS "Submission_Losses" integer,
  ADD COLUMN IF NOT EXISTS "Decision_Wins" integer,
  ADD COLUMN IF NOT EXISTS "Decision_Losses" integer,
  ADD COLUMN IF NOT EXISTS "TapologyEventURL" text,
  ADD COLUMN IF NOT EXISTS "TapologyFighterURL" text,
  ADD COLUMN IF NOT EXISTS "TapologyMatchConfidence" text;

CREATE SEQUENCE IF NOT EXISTS public.ufc_full_fight_card_id_seq AS bigint;

DO $$
DECLARE
  current_max_id bigint;
BEGIN
  IF to_regclass('public.ufc_full_fight_card') IS NULL THEN
    RAISE EXCEPTION 'public.ufc_full_fight_card does not exist';
  END IF;

  ALTER SEQUENCE public.ufc_full_fight_card_id_seq
    OWNED BY public.ufc_full_fight_card.id;

  ALTER TABLE public.ufc_full_fight_card
    ALTER COLUMN id SET DEFAULT nextval('public.ufc_full_fight_card_id_seq'::regclass);

  SELECT GREATEST(COALESCE(MAX(id), 0), 1024)
  INTO current_max_id
  FROM public.ufc_full_fight_card;

  PERFORM setval('public.ufc_full_fight_card_id_seq'::regclass, current_max_id, true);
END;
$$;
