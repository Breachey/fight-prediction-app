-- Automation-only import: remove picks for withdrawn bouts atomically with the card.
CREATE FUNCTION public.reconcile_upcoming_fight_card_event(
  p_event_id bigint,
  p_event_name text,
  p_event_date date,
  p_venue text,
...
BEGIN
    ...
    OR EXISTS (
      ...
      WHERE c."EventId" = p_event_id
        AND (NULLIF(BTRIM(c."StartTime"::text), '')::timestamptz <= clock_timestamp()
          OR EXISTS (SELECT 1 FROM public.fight_results r WHERE r.fight_id = c."FightId"))
    ) THEN
  ...
  IF EXISTS (
    SELECT 1 FROM public.ufc_full_fight_card c
    JOIN public.predictions p ON p.fight_id = c."FightId"
    WHERE c."EventId" = p_event_id
  ...
  DELETE FROM public.predictions WHERE fight_id = ANY(removed_fight_ids);
